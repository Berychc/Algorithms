...
public class AlgorithmsLogic {
     ...
     * Один муравей движется влево, другой движется вправо. В одном раунде каждый муравей делает ход.
     * Муравей, идущий влево, движется первым.
     *
     * Если муравей собирается двигаться в точку, которая уже занята другим муравьем,
     * то каждый из муравьев меняет направление, и только после этого движется текущий муравей.
     * Предположим, что смена направления не занимает дополнительного раунда.
     * Также муравьи не могут занимать одно и то же положение на доске.
     *
     * Когда муравей достигает одного конца доски и движется к ее концу, он не падает с доски сразу,
     * это происходит в следующем раунде.
     *
     * Дано целое число n — длина доски и два целых числа: left — положение муравья, который движется влево,
     * и right — положение муравья, который движется вправо. Верните количество раундов, необходимых для того,
     * чтобы первый муравей выпал с доски.
     */
    ...
    public static int roundsToFallOff1(int n, int left, int right) {
        int rounds = 0;
        ...
        return Math.min(n - left, right);
    }

    public static int roundsToFallOff2(int n, int left, int right) {
        int timeToFallRight = n - right + 1;
